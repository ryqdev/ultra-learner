...
The PDF.js browser entry and worker both use the library's legacy distribution so PDF.js supplies compatibility shims for browsers without its newest required JavaScript APIs. The paired assets must remain on the same PDF.js version.

The reader validates the selected file before reading it. PDF.js parses a copy in browser memory; after parsing succeeds, the original bytes are posted to the same-origin session API for local persistence. No account or cloud service is involved, and the server binds to loopback by default. Text PDFs receive a PDF.js `TextLayerBuilder` positioned over the canvas, so browser selection and copying use the source text without changing the visual page. Image-only pages continue to render through the canvas but do not produce text selection context. PDF.js failures are normalized into concise reader guidance, while unexpected failures retain a bounded diagnostic instead of being presented as invalid-file errors.

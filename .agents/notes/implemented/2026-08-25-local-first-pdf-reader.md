...
The first Ultra Learner product is a single-process Bun application with a local-first browser reader. Bun serves the static shell, bundles the TypeScript browser entry on request, and provides the PDF.js worker assets. PDF.js is the sole runtime dependency and parses selected file bytes in a web worker. The browser entry and its matching worker use PDF.js's legacy distribution so browsers missing newly standardized JavaScript collection and typed-array methods receive PDF.js's own compatibility shims. The initial reader kept selected documents only in browser memory; the later [local PDF session history](2026-08-26-local-pdf-session-history.md) decision supersedes that ephemeral document boundary while retaining browser-owned parsing and rendering.
...
PDF.js parsing and rendering failures retain a short, normalized diagnostic in the reader instead of being flattened into an invalid-file message. Known password, invalid-document, and browser-feature failures still receive concise guidance, while unexpected errors expose enough detail to diagnose the browser integration without opening developer tools.
...
**Use PDF.js's modern distribution and require only the newest browsers.** The modern bundle is smaller, but its unpolyfilled use of newly standardized APIs can reject otherwise valid PDFs before rendering. The legacy distribution keeps parsing behavior aligned while making the local reader usable in embedded and slightly older browsers.
...
- The browser bundle and worker are larger because they include PDF.js compatibility shims, and both assets must remain on the same legacy distribution and version.

...
The repository uses TypeScript for executable product code, tests, and validation scripts. Bun is the sole dependency manager, script runner, test runner, application runtime, and lockfile owner. The later [release automation decision](2026-09-16-release-pr-automation.md) adds a narrow exception for authenticated npm uploads in CI; dependency installation, checks, and packing remain Bun-owned. A lockfile is retained when product dependencies exist and is omitted when there are none. The repository owns this structure and its decisions independently; no external project's names, history, or conventions are part of its public identity. Product-specific architecture is allowed to evolve independently while preserving this agent-facing foundation.
